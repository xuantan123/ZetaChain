...
contract ZetaRouter is IZetaRouter02 {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = ZetaLibrary.getReserves(factory, tokenA, tokenB);
    ...
    function removeLiquidity(
        ...
        (uint256 amount0, uint256 amount1) = IZetaPair(pair).burn(to);
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            ZetaLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = ZetaLibrary.sortTokens(input, output);
            IZetaPair pair = IZetaPair(ZetaLibrary.pairFor(factory, input, output));
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountOutput = ZetaLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint256 amount0Out, uint256 amount1Out) =
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            "ZetaRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    }
    ...
    function getAmountIn(
        ...
        return ZetaLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    }
