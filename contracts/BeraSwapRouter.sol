...
library BeraSwapLibrary {
    using SafeMath for uint256;

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'BeraSwapLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'BeraSwapLibrary: ZERO_ADDRESS');
    }
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IBeraSwapPair(pairFor(factory, tokenA, tokenB)).getReserves();
    ...
    function quote(
        ...
        require(amountA > 0, 'BeraSwapLibrary: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'BeraSwapLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, 'BeraSwapLibrary: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'BeraSwapLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, 'BeraSwapLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'BeraSwapLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(
        ...
        require(path.length >= 2, 'BeraSwapLibrary: INVALID_PATH');
...
contract BeraSwapRouter is IBeraSwapRouter {
    ...
    function _addLiquidity(
        ...
        if (IBeraSwapFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IBeraSwapFactory(factory).createPair(tokenA, tokenB);
        }
        (uint256 reserveA, uint256 reserveB) = BeraSwapLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint256 amountBOptimal = BeraSwapLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'BeraSwapRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = BeraSwapLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'BeraSwapRouter: INSUFFICIENT_A_AMOUNT');
        ...
        )
    {
        ...
        address pair = BeraSwapLibrary.pairFor(factory, tokenA, tokenB);
    ...
    function removeLiquidity(
        ...
        address pair = BeraSwapLibrary.pairFor(factory, tokenA, tokenB);
        IBeraSwapPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint256 amount0, uint256 amount1) = IBeraSwapPair(pair).burn(to);
        (address token0, ) = BeraSwapLibrary.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'BeraSwapRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (address token0, ) = BeraSwapLibrary.sortTokens(input, output);
            uint256 amountOut = amounts[i + 1];
            (uint256 amount0Out, uint256 amount1Out) = input == token0
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            address to = i < path.length - 2 ? BeraSwapLibrary.pairFor(factory, output, path[i + 2]) : _to;
            IBeraSwapPair(BeraSwapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to);
        }
    }
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            BeraSwapLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = BeraSwapLibrary.sortTokens(input, output);
            IBeraSwapPair pair = IBeraSwapPair(BeraSwapLibrary.pairFor(factory, input, output));
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountOutput = BeraSwapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint256 amount0Out, uint256 amount1Out) = input == token0
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'BeraSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    ...
    function quote(
        ...
        return BeraSwapLibrary.quote(amountA, reserveA, reserveB);
    }
    ...
    function getAmountOut(
        ...
        return BeraSwapLibrary.getAmountOut(amountIn, reserveIn, reserveOut);
    }
    ...
    function getAmountIn(
        ...
        return BeraSwapLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    }
    ...
    {
        return BeraSwapLibrary.getAmountsOut(factory, amountIn, path);
    }
    ...
    function getAmountsIn(uint256 amountOut, address[] memory path)
        ...
        returns (uint256[] memory amounts)
    {
        return BeraSwapLibrary.getAmountsIn(factory, amountOut, path);
    }
}