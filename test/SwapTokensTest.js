...
// Contract addresses - using the same ones from your AddLiquidityTest.js
const ROUTER_ADDRESS = "0xD49C337715128e79868d9AC18C4227e91156108F";
...
// ABIs - reusing from your existing code
const routerABI = [
  {
    "inputs": [
      {
...
async function checkBalances() {
  try {
    ...
    console.log(`\n💰 Số dư token:`);
    console.log(`- WZETA: ${ethers.utils.formatUnits(balanceA, decimalsA)}`);
    console.log(`- ZTW: ${ethers.utils.formatUnits(balanceB, decimalsB)}`);
    
    return { balanceA, balanceB, decimalsA, decimalsB };
  } catch (error) {
    console.error(`❌ Lỗi khi kiểm tra số dư:`, error.message);
    return { balanceA: 0, balanceB: 0, decimalsA: 18, decimalsB: 18 };
  }
}
...
async function checkPair() {
  try {
    ...
    if (pairAddress === ethers.constants.AddressZero) {
      console.error(`❌ Cặp token chưa được tạo!`);
      return false;
    }
    
    console.log(`✅ Cặp token đã tồn tại.`);
    return true;
  } catch (error) {
    console.error(`❌ Lỗi khi kiểm tra cặp:`, error.message);
    return false;
  }
}
...
async function approveTokens() {
  try {
    ...
    // Check current allowance for WZETA
    const allowanceA = await tokenContractA.allowance(userAddress, ROUTER_ADDRESS);
    if (allowanceA.lt(amountToApprove)) {
      console.log(`- Đang cấp quyền cho WZETA...`);
      const txA = await tokenContractA.approve(ROUTER_ADDRESS, amountToApprove);
      await txA.wait();
      console.log(`✅ Đã cấp quyền cho WZETA thành công!`);
    } else {
      console.log(`✅ Đã có quyền cho WZETA.`);
    }
    
    // Check current allowance for ZTW
    const allowanceB = await tokenContractB.allowance(userAddress, ROUTER_ADDRESS);
    if (allowanceB.lt(amountToApprove)) {
      console.log(`- Đang cấp quyền cho ZTW...`);
      const txB = await tokenContractB.approve(ROUTER_ADDRESS, amountToApprove);
      await txB.wait();
      console.log(`✅ Đã cấp quyền cho ZTW thành công!`);
    } else {
      console.log(`✅ Đã có quyền cho ZTW.`);
    }
    
    return true;
  } catch (error) {
    console.error(`❌ Lỗi khi cấp quyền:`, error.message);
    return false;
  }
}
...
// Main function
async function main() {
  console.log(`\n🚀 Bắt đầu chương trình swap token...`);
  
  // Check initial balances
  await checkBalances();
  
  // Check if pair exists
  const pairExists = await checkPair();
  if (!pairExists) {
    console.error(`❌ Không thể swap vì cặp token chưa tồn tại!`);
    return;
  }
  ...
  if (!approved) {
    console.error(`❌ Không thể swap vì cấp quyền thất bại!`);
    return;
  }
  ...
  if (success1) {
    // If successful, wait 10 seconds then swap back
  ...
  console.log(`\n🏁 Chương trình swap token hoàn tất.`);
}

// Run the program
main().catch(error => {
  console.error(`\n❌ Lỗi chính:`, error.message);
}); 